...
impl LoggingTransformer {
    ...
    pub fn log_vault_operation(operation: &str, key: &str, success: bool) {
        ...
        if success {
            info!("Vault operation succeeded: {} (key_hash: {})", operation, key_hash);
        } else {
            warn!("Vault operation failed: {} (key_hash: {})", operation, key_hash);
        }
    }
    ...
    pub fn log_auth_event(event: &str, user_id: Option<&str>, success: bool) {
        if success {
            let user_hash = user_id
                .map(Self::secure_hash_key)
                .unwrap_or_else(|| "anonymous".to_string());
            info!("Authentication succeeded: {} (user_hash: {})", event, user_hash);
        } else {
    ...
    pub fn log_performance_metric(operation: &str, duration_ms: u64, success: bool) {
        if success {
            ...
            warn!("Performance: {} failed after {}ms", operation, duration_ms);
        }
    }

    /// Log network operations with connection details
    pub fn log_network_operation(operation: &str, remote_addr: &str, bytes_transferred: Option<usize>) {
        match bytes_transferred {
            Some(bytes) => debug!("Network: {} to {} ({} bytes)", operation, remote_addr, bytes),
            None => debug!("Network: {} to {}", operation, remote_addr),
        }
    }
...
macro_rules! log_crypto_safe {
    (ERROR, $operation:expr, $error:expr) => {
        log::error!("Cryptographic operation failed: {} (error_type: {})", 
            $operation, std::any::type_name_of_val(&$error)
        );
    };
    (WARN, $operation:expr, $error:expr) => {
        log::warn!("Cryptographic operation warning: {} (error_type: {})", 
            $operation, std::any::type_name_of_val(&$error)
        );
    };
}
