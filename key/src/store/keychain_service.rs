...
pub enum KeychainRequest {
    Store { service_name: String, key_id: String, data: Vec<u8> },
    Retrieve { service_name: String, key_id: String },
    Delete { service_name: String, key_id: String },
    Exists { service_name: String, key_id: String },
}
...
impl KeychainServiceManager {
    pub async fn new() -> Result<Self, KeyError> {
        ...
        pattern.start_handler(|req| async move {
            match req {
                KeychainRequest::Store { service_name, key_id, data } => {
                    // Direct keychain operations without Parker
                    Self::perform_store(&service_name, &key_id, &data).map(|_| KeychainResponse::Stored)
                }
                KeychainRequest::Retrieve { service_name, key_id } => {
                    Self::perform_retrieve(&service_name, &key_id).map(KeychainResponse::Retrieved)
                }
                KeychainRequest::Delete { service_name, key_id } => {
                    Self::perform_delete(&service_name, &key_id).map(|_| KeychainResponse::Deleted)
                }
                KeychainRequest::Exists { service_name, key_id } => {
                    Self::perform_exists(&service_name, &key_id).map(KeychainResponse::Exists)
                }
            }
        }).await.map_err(|e| KeyError::Internal(format!("Failed to start keychain handler: {}", e)))?;
        
        Ok(Self { pattern })
    }
    
    pub async fn store(&self, service_name: String, key_id: String, data: Vec<u8>) -> Result<(), KeyError> {
        let request = KeychainRequest::Store { service_name, key_id, data };
        match self.pattern.request(request).await.map_err(|e| KeyError::Internal(e.to_string()))? {
    ...
    pub async fn retrieve(&self, service_name: String, key_id: String) -> Result<Vec<u8>, KeyError> {
        let request = KeychainRequest::Retrieve { service_name, key_id };
        match self.pattern.request(request).await.map_err(|e| KeyError::Internal(e.to_string()))? {
    ...
    pub async fn delete(&self, service_name: String, key_id: String) -> Result<(), KeyError> {
        let request = KeychainRequest::Delete { service_name, key_id };
        match self.pattern.request(request).await.map_err(|e| KeyError::Internal(e.to_string()))? {
    ...
    pub async fn exists(&self, service_name: String, key_id: String) -> Result<bool, KeyError> {
        let request = KeychainRequest::Exists { service_name, key_id };
        match self.pattern.request(request).await.map_err(|e| KeyError::Internal(e.to_string()))? {
    ...
    pub async fn list(&self, _service_name: String, _pattern: String) -> Result<Vec<String>, KeyError> {
