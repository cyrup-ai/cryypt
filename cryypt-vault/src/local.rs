...
impl LocalVaultProvider {
    ...
    fn derive_key(&self, passphrase: &str, salt: &[u8]) -> VaultResult<Zeroizing<Vec<u8>>> {
    ...
    async fn encrypt_data(&self, data: &[u8], key: &[u8]) -> VaultResult<Vec<u8>> {
        ...
        use cryypt_key::{traits::KeyProviderBuilder, KeyResult};
        ...
        let key_provider = RawKeyProvider {
            key: key.to_vec(),
        };
        ...
        match self.cipher_algorithm {
            ...
            CipherAlgorithm::Cascade => {
                ...
                if key.len() < 64 {
                    return Err(VaultError::Encryption("Cascade requires 64-byte key".into()));
                }
            ...
            CipherAlgorithm::Custom(ref name) => {
                Err(VaultError::Encryption(format!("Custom cipher '{}' not implemented", name)))
            }
        }
    }
    ...
    async fn decrypt_data(&self, encrypted_data: &[u8], key: &[u8]) -> VaultResult<Vec<u8>> {
        ...
        match self.cipher_algorithm {
            ...
            CipherAlgorithm::Cascade => {
                // For cascade, we need to split the key for two passes
                if key.len() < 64 {
                    return Err(VaultError::Decryption("Cascade requires 64-byte key".into()));
                }
            ...
            CipherAlgorithm::Custom(ref name) => {
                Err(VaultError::Decryption(format!("Custom cipher '{}' not implemented", name)))
            }
        }
    }
