...
// Commented out as we'll use a different approach
// impl<T> From<Option<T>> for Error {
//     fn from(_: Option<T>) -> Self {
//         Error::Database("Empty database response".to_string())
//     }
// }
    ...
    T: Serialize + DeserializeOwned + Send + Sync + 'static,
{
        ...
        'life2: 'fut,
    {
        ...
        tokio::spawn(async move {
            ...
            {
                Ok(mut response) => {
                    match response.take::<Vec<T>>((0_usize, "")) {
                        Ok(items) => {
                            for item in items {
        ...
        'life2: 'fut,
    {
        ...
        let db = Arc::clone(self.db());
        let table = self.table_name().to_string();
        let other_table = other_table.to_string();
        let join_field = join_field.to_string();
