    ...
    match operation.operation_type {
        OperationType::Insert | OperationType::Update => {
            if let Some(encrypted_value) = operation.encrypted_value {
                ...
                // Validate timestamp before conversion - no silent fallbacks
                let created_at = chrono::DateTime::from_timestamp(operation.timestamp as i64, 0)
                    .ok_or_else(|| VaultError::InvalidInput(format!("Invalid timestamp: {}", operation.timestamp)))?;
                ...
                let result: Result<Option<VaultEntry>, surrealdb::Error> = db_clone
                    ...
                    .content(VaultEntry {
                        ...
                        key: serde_json::to_string(&operation.key)
                            .unwrap_or_else(|_| format!("key_{}", operation.timestamp)),
                ...
                result.map_err(|e| {
                    log::warn!("Cache persistence failed for key {:?}: {}", operation.key, e);
