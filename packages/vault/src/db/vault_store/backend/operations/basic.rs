...
impl LocalVaultProvider {
    ...
    pub async fn put(&self, key: &str, value: &VaultValue) -> VaultResult<()> {
        ...
        self.put_impl(key.to_string(), value.clone(), None).await
    }
    ...
    pub(crate) async fn put_impl(
        ...
        let created: Vec<VaultEntry> = result.take(0).map_err(|e| {
            VaultError::Provider(format!("Failed to check create/update result: {e}"))
        })?;

        log::debug!("PUT: Operation returned {} records", created.len());
        if !created.is_empty() {
            if let Some(id) = &created[0].id {
                log::debug!("PUT: Successfully stored record with ID: {}", id);
            }
        }
    ...
    pub(crate) async fn delete_impl(&self, key: &str, namespace: Option<&str>) -> VaultResult<()> {
        ...
        log::debug!(
            "DELETE: Attempting deletion with record ID: {}",
            record_id
        );
        ...
        log::debug!("DELETE: Operation returned {} records", deleted_records.len());
