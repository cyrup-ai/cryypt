...
impl LocalVaultProvider {
    ...
    pub async fn re_encrypt_with_new_passphrase(
        ...
        for entry in entries {
            ...
            let encrypted_bytes = base64::Engine::decode(&base64::engine::general_purpose::STANDARD, &entry.value)
            ...
            let value_b64 = base64::Engine::encode(&base64::engine::general_purpose::STANDARD, re_encrypted_bytes);
            
            // Update entry in database with new encrypted value
    ...
    pub(crate) async fn decrypt_data_with_passphrase(&self, encrypted_data: &[u8], passphrase: &str) -> VaultResult<Vec<u8>> {
    ...
    pub(crate) async fn encrypt_data_with_passphrase(&self, data: &[u8], passphrase: &str) -> VaultResult<Vec<u8>> {
        // Derive encryption key from the specified passphrase
        use crate::operation::Passphrase;
        let passphrase_secret = Passphrase::new(passphrase.to_string().into());
        let key = self.derive_encryption_key(&passphrase_secret).await?;
