...
use base64::{Engine as _, engine::general_purpose::STANDARD as BASE64_STANDARD, engine::general_purpose::URL_SAFE_NO_PAD as BASE64_URL};
...
impl LocalVaultProvider {
    ...
    pub(crate) async fn find_impl(&self, pattern: &str) -> VaultResult<Vec<(String, VaultValue)>> {
        ...
        log::debug!("FIND: Successfully extracted {} entries from DB", entries.len());
        ...
        for entry in entries {
            ...
            if let Ok(decoded_bytes) = BASE64_URL.decode(&key) {
                if let Ok(decoded_str) = String::from_utf8(decoded_bytes) {
                    if decoded_str.starts_with("__vault_") {
                        log::debug!("FIND: Skipping system entry (base64 encoded): {} -> {}", key, decoded_str);
                        continue;
                    }
                }
            }
