...
impl LocalVaultProvider {
    ...
    pub(crate) async fn restore_jwt_session(&self) -> VaultResult<Option<(String, Vec<u8>)>> {
        ...
        let session_record: Option<JwtSessionRecord> = result.take(0).map_err(|e| {
            VaultError::Provider(format!("Failed to deserialize JWT session: {e}"))
        })?;
    ...
    pub(crate) async fn touch_jwt_session(&self, vault_path_hash: &str) -> VaultResult<()> {
        ...
        db.query(&update_query)
            .bind(("last_accessed", surrealdb::value::Datetime::from(chrono::Utc::now())))
            .await
            .map_err(|e| VaultError::Provider(format!("Failed to update session access time: {e}")))?;
    ...
    pub(crate) async fn delete_jwt_session(&self, vault_path_hash: &str) -> VaultResult<()> {
        ...
        log::debug!("DELETE_SESSION: Removed JWT session for vault hash: {}", vault_path_hash);
        Ok(())
    }
    ...
    pub(crate) async fn cleanup_expired_sessions(&self) -> VaultResult<()> {
        ...
        let mut result = db
            ...
            .await
            .map_err(|e| VaultError::Provider(format!("Failed to cleanup expired sessions: {e}")))?;
        ...
        if count > 0 {
            log::info!("CLEANUP_SESSIONS: Cleaned up {} expired JWT sessions", count);
        } else {
    ...
    pub(crate) async fn populate_session_state(
        ...
        *jwt_key_guard = Some(jwt_key);
        ...
        log::info!("POPULATE_SESSION: Successfully restored JWT session state (vault remains locked until passphrase provided)");
        Ok(())
    }
