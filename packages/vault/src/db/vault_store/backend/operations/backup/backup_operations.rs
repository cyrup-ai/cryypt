...
impl LocalVaultProvider {
    ...
    pub async fn create_encrypted_backup(&self, backup_passphrase: &str) -> VaultResult<Vec<u8>> {
        ...
        let entries: Vec<VaultEntry> = db.select("vault_entries").await.map_err(|e| {
            VaultError::Provider(format!("Failed to get entries for backup: {e}"))
        })?;
        ...
        let backup_json =
            serde_json::to_vec(&backup_data).map_err(|e| VaultError::Serialization(e))?;
    ...
    pub async fn restore_from_encrypted_backup(
        ...
        let backup_data: BackupData =
            serde_json::from_slice(&backup_json).map_err(|e| VaultError::Serialization(e))?;
