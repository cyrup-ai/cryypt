...
use crate::db::{DocumentDao, Document};
use std::sync::Arc;
...
impl PassInterface {
    ...
    pub async fn from_path<P: AsRef<Path>>(store_path: P) -> VaultResult<Self> {
        ...
        let db = surrealdb::engine::any::connect(db_url.as_str())
            .await
            .map_err(|e| crate::error::VaultError::Provider(format!("Database connection failed: {}", e)))?;
            
        db.use_ns("passwords").use_db("store")
            .await
            .map_err(|e| crate::error::VaultError::Provider(format!("Database setup failed: {}", e)))?;
    ...
    pub async fn list(&self) -> VaultResult<Vec<String>> {
        let documents = self.dao.find_by_tag("password".to_string())
            .await
            .map_err(|e| crate::error::VaultError::Provider(format!("Failed to find password entries: {}", e)))?;
        Ok(documents.into_iter().map(|doc| doc.key).collect())
    }

    /// Get a specific password entry
    pub async fn get(&self, name: &str) -> VaultResult<String> {
        match self.dao.find_by_key(name.to_string())
            .await
            .map_err(|e| crate::error::VaultError::Provider(format!("Failed to get password entry: {}", e)))? {
    ...
    pub async fn search(&self, query: &str) -> VaultResult<Vec<String>> {
        ...
        Ok(all.into_iter().filter(|entry| entry.contains(query)).collect())
    }
    ...
    pub async fn insert(&self, name: &str, password: &str) -> VaultResult<()> {
        ...
        self.dao.save(document)
            .await
            .map_err(|e| crate::error::VaultError::Provider(format!("Failed to insert password: {}", e)))?;
        Ok(())
    }

    /// Remove a password entry
    pub async fn remove(&self, name: &str) -> VaultResult<()> {
        let deleted = self.dao.delete_by_key(name.to_string())
            .await
            .map_err(|e| crate::error::VaultError::Provider(format!("Failed to delete password: {}", e)))?;
