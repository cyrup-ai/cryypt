...
impl JwtHandler {
    ...
    pub async fn create_jwt_token(
        ...
        // Sign JWT token
        let token = jwt_builder.sign(claims).await
    ...
    pub async fn validate_jwt_token(
        ...
        let claims_value = jwt_builder.verify(token.to_string()).await
            .map_err(|e| match e {
                JwtError::TokenExpired => VaultError::AuthenticationFailed("JWT token expired. Please login again.".to_string()),
                JwtError::InvalidSignature => VaultError::AuthenticationFailed("Invalid JWT signature".to_string()),
                JwtError::InvalidToken(msg) => VaultError::AuthenticationFailed(format!("Invalid JWT token: {}", msg)),
                _ => VaultError::AuthenticationFailed(format!("JWT validation failed: {}", e)),
            })?;

        // Parse claims into our structure
        let claims: VaultJwtClaims = serde_json::from_value(claims_value)
            .map_err(|e| VaultError::AuthenticationFailed(format!("Invalid JWT claims structure: {}", e)))?;
    ...
    fn derive_jwt_secret(&self, master_key: &[u8]) -> VaultResult<Vec<u8>> {
        ...
        let combined_input = format!("{}:{}", 
            BASE64_STANDARD.encode(master_key), 
            jwt_context
        );
        ...
        let jwt_secret_bytes = master_key_provider.resolve()
    ...
    fn validate_vault_claims(&self, claims: &VaultJwtClaims) -> VaultResult<()> {
        ...
        uuid::Uuid::parse_str(&claims.session_id)
            .map_err(|_| VaultError::AuthenticationFailed(
                "Invalid session ID format in JWT claims".to_string()
            ))?;
...
mod tests {
    ...
    async fn test_jwt_creation_and_validation() {
        ...
        // Validate JWT token
        let claims = handler.validate_jwt_token(&token, master_key).await.unwrap();
    ...
    async fn test_vault_id_validation() {
        ...
        let token = handler1.create_jwt_token(master_key, Some(1)).await.unwrap();

        // Should fail validation with vault_2 (different vault_id)
        let result = handler2.validate_jwt_token(&token, master_key).await;
        assert!(result.is_err());
        assert!(result.unwrap_err().to_string().contains("not valid for this vault"));
    }
