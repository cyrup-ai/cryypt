...
impl ClaimsBuilder {
    ...
    pub fn expires_in(mut self, duration: Duration) -> Self {
        ...
        self.claims.insert("exp".to_string(), Value::Number(exp.into()));
        self
    }
    ...
    pub fn issued_now(mut self) -> Self {
        ...
        self.claims.insert("iat".to_string(), Value::Number(iat.into()));
        self
    }
    ...
    pub fn audience(mut self, audience: Vec<String>) -> Self {
        ...
        self.claims.insert("aud".to_string(), Value::Array(aud_values));
        self
    }
    ...
    pub fn not_before(mut self, nbf: chrono::DateTime<Utc>) -> Self {
        self.claims.insert("nbf".to_string(), Value::Number(nbf.timestamp().into()));
        self
    }
    ...
    pub fn jwt_id(mut self, jti: &str) -> Self {
        self.claims.insert("jti".to_string(), Value::String(jti.to_string()));
        self
    }
    ...
    pub fn build(self) -> Claims {
        ...
        let iat = self.claims.get("iat")
            .and_then(serde_json::Value::as_i64)
            .unwrap_or(0);

        // Extract optional fields
        let iss = self.claims.get("iss")
            .and_then(|v| v.as_str())
            .map(std::string::ToString::to_string);

        let aud = self.claims.get("aud")
        ...
        let nbf = self.claims.get("nbf")
            .and_then(serde_json::Value::as_i64);

        let jti = self.claims.get("jti")
            .and_then(|v| v.as_str())
            .map(std::string::ToString::to_string);

        // Collect extra fields (all fields not in standard claims)
        let standard_fields = ["sub", "exp", "iat", "iss", "aud", "nbf", "jti"];
        let extra: HashMap<String, Value> = self.claims.into_iter()
