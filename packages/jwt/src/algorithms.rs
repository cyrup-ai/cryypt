...
pub fn generate_es256_keypair() -> crate::error::JwtResult<Es256KeyPair> {
    ...
    let signing_key = SigningKey::from_slice(&private_key_bytes)
        .map_err(|e| crate::error::JwtError::InvalidKey(format!("Failed to create signing key: {}", e)))?;
...
pub fn validate_es256_keypair(keypair: &Es256KeyPair) -> bool {
    ...
    let private_key_valid = keypair.private_key.len() == 32
        && SigningKey::from_slice(&keypair.private_key).is_ok();
