...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    while let Some(chunk) = stream.next().await {
        ...
        println!("Received compressed chunk, total size so far: {}", total_compressed_size);
    }

    println!("Streaming compression complete: {} bytes -> {} bytes", 
             test_data.len(), total_compressed_size);
    ...
    println!("Decompression complete: {} bytes -> {} bytes", 
             compressed_size, final_decompressed.len());
