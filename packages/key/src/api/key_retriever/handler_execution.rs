            ...
            let key_bytes = store.retrieve(&simple_key_id)
                .on_result(|storage_result| match storage_result {
                    ...
                    Err(storage_error) => {
                        ...
                        operation_status = if error_msg.contains("not found") || error_msg.contains("No such key") {
                        ...
                        tracing::error!("Key retrieval failed for {} with status {:?}: {}", 
                            simple_key_id.id(), operation_status, storage_error);
