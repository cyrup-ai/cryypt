...
//!
//! This module provides SHA3-256 hash computation with both regular hashing
...
use super::core::{Sha3_256Hasher, DynHasher};
    ...
    pub fn compute_stream<S: Stream<Item = Vec<u8>> + Send + 'static>(
        ...
        tokio::spawn(async move {
            ...
            let mut mac = Hmac::<Sha3_256>::new_from_slice(&key)
                .map_err(|e| HashError::MacInitialization(format!("Failed to initialize SHA3-256 HMAC: {e}")))?;
...
pub async fn sha3_256_hash(data: Vec<u8>, salt: Option<Vec<u8>>, passes: u32) -> Result<Vec<u8>> {
    use sha3::{Digest, Sha3_256};
...
pub async fn sha3_256_hmac(data: Vec<u8>, key: Vec<u8>) -> Result<Vec<u8>> {
    use hmac::{Hmac, Mac};
    use sha3::Sha3_256;
