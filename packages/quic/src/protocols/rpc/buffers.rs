...
use super::types::{RpcError, Result};
...
impl ConnectionBuffers {
    ...
    pub fn process_request(&mut self, data: &mut [u8]) -> Result<OwnedValue> {
        ...
        if data_len > 16 * 1024 * 1024 {  // 16MB limit
        ...
        self.total_bytes_processed.fetch_add(data_len as u64, Ordering::Relaxed);
        ...
        if (estimated_capacity as u64) > current_peak {
            self.peak_buffer_size.store(estimated_capacity as u64, Ordering::Relaxed);
        }
    ...
    pub fn reset(&mut self) {
        ...
        self.peak_buffer_size.store(self.initial_capacity as u64, Ordering::Relaxed);
...
mod tests {
    ...
    fn test_batch_processing() {
        ...
        let mut chunks = vec![json1.as_mut_slice(), json2.as_mut_slice(), json3.as_mut_slice()];
