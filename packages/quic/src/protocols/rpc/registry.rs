...
use super::types::{Id, Params, RpcError, Result};
...
impl JsonRpcRegistry {
    ...
    pub async fn call_method(
        ...
        let method = self.methods.get(method_name).ok_or_else(|| {
            RpcError::method_not_found(method_name.to_string())
        })?;

        // Create context
        let mut ctx = RpcContext::new(request_id.clone(), method_name.to_string(), self.default_timeout);
    ...
    pub fn register_introspection_methods(&self) {
        ...
        self.register("echo", |_ctx, params| async {
            match params {
                ...
                Some(Params::Object(mut obj)) if obj.contains_key("message") => {
                    Ok(obj.remove("message").unwrap_or(OwnedValue::from("no message")))
                }
    ...
    pub fn register_default_introspection_methods(&self) {
        ...
        if !registered_methods.is_empty() {
            info!("Registered default introspection methods: {}", registered_methods.join(", "));
        }
    }
}
...
mod tests {
    ...
    async fn test_method_registration_and_call() {
        ...
        registry.register("add", |_ctx, params| async move {
            match params {
                ...
                _ => Err(RpcError::invalid_params("Expected array of two numbers".to_string())),
            }
        });
        ...
        let result = registry.call_method("add", params, Id::Number(1), None).await;
        assert!(result.is_ok());
        assert_eq!(result.unwrap().as_i64(), Some(8));

        // Test method not found
        let result = registry.call_method("subtract", None, Id::Number(2), None).await;
    ...
    async fn test_introspection_methods() {
        ...
        // Test ping
        let result = registry.call_method("ping", None, Id::Number(1), None).await;
        assert!(result.is_ok());
        assert_eq!(result.unwrap().as_str(), Some("pong"));

        // Test echo
        let params = Some(Params::Array(vec![OwnedValue::from("hello")]));
        let result = registry.call_method("echo", params, Id::Number(2), None).await;
        assert!(result.is_ok());
    ...
    async fn test_request_tracking() {
        ...
        let handle1 = tokio::spawn({
            let registry = registry.clone();
            async move {
                registry.call_method("slow", None, Id::Number(1), None).await
            }
        });
        ...
        let handle2 = tokio::spawn({
            let registry = registry.clone();
            async move {
                registry.call_method("slow", None, Id::Number(2), None).await
            }
        });
    ...
    async fn test_concurrent_limit() {
        ...
        let result2 = registry.call_method("slow", None, Id::Number(2), None).await;
