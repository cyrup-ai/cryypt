...
use super::{
    ...
    types::{BatchRequest, BatchResponse, Id, Request, Response, RpcResponse, RpcError, Result},
};
...
impl RequestProcessor {
    ...
    pub async fn process_raw_data(
        ...
        let parsed_value = buffers.process_request(&mut data)
            .map_err(|e| {
        ...
        match value {
            OwnedValue::Array(array) => {
                ...
                for item in array.into_iter() {
                    let request: Request = serde_json::from_value(serde_json::to_value(item)
                        .map_err(|e| RpcError::parse_error(format!("Value conversion failed: {e}")))?)
                        .map_err(|e| RpcError::parse_error(format!("Request parsing failed: {e}")))?;
                    requests.push(request);
                }
                let batch_request = BatchRequest::new(requests);
                self.process_batch_request(batch_request, connection_id).await
            }
            OwnedValue::Object(_) => {
                // Single request processing
                let request: Request = serde_json::from_value(serde_json::to_value(value)
                    .map_err(|e| RpcError::parse_error(format!("Value conversion failed: {e}")))?)
                    .map_err(|e| RpcError::parse_error(format!("Request parsing failed: {e}")))?;
    ...
    async fn process_single_request(
        ...
        if request.id.is_none() {
            ...
            if let Err(e) = self.registry.call_method(
                &request.method,
                request.params,
            ...
            return Err(RpcError::internal_error("Notification processed".to_string()));
        }
        ...
        // Call the method
        let result = self.registry.call_method(
            &request.method,
            request.params,
            request_id.clone(),
            connection_id,
        ...
        match result {
            Ok(result_value) => {
                Ok(Response::success(result_value, request_id))
            }
            Err(error) => {
    ...
    async fn process_batch_request(
        ...
        for request in batch_request.0 {
            ...
            let task = tokio::spawn(async move {
                processor.process_single_request(request, conn_id).await
            });
        ...
        for task in tasks {
            match task.await {
                ...
                Err(join_error) => {
                    ...
                    responses.push(Response::error(
                        RpcError::internal_error("Task execution failed".to_string()).to_error_object(),
                        Id::Null,
                    ));
                }
            }
        ...
        if responses.is_empty() {
            return Err(RpcError::internal_error("All requests were notifications".to_string()));
        }
        ...
        let json_value = match response {
            ...
            RpcResponse::Batch(responses) => {
                serde_json::to_value(responses)
                    .map_err(|e| RpcError::internal_error(format!("Batch serialization failed: {e}")))?
            }
        };
