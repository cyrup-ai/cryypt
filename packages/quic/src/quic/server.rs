...
impl ServerListenerBuilder {
    ...
    async fn run_packet_processing_loop(
        ...
        connections: Arc<Mutex<HashMap<quiche::ConnectionId<'static>, Arc<QuicConnectionController>>>>,
        buf: &mut [u8],
        out: &mut [u8; 1350],
    ) -> Result<()> {
        use crate::error::QuicError;
        ...
        loop {
            match socket.recv_from(buf).await {
                Ok((len, from)) => {
                    ...
                    // Parse QUIC header
                    let Ok(hdr) = quiche::Header::from_slice(pkt_buf, quiche::MAX_CONN_ID_LEN) else { 
                        continue 
                    };
    ...
    async fn process_packet_data(
        ...
        let recv_info = quiche::RecvInfo { from: context.from, to: local_addr };
        ...
        for (data, to) in should_send_responses {
            context.socket.send_to(&data, to).await.map_err(QuicError::Network)?;
        }
        ...
        if !readable_streams.is_empty() {
            ...
            for (stream_id, request_data) in pending_rpc_requests {
                self.process_stream_request_async(stream_id, request_data, controller).await;
            }
        }
        ...
        if is_closed {
            let mut conns = connections.lock().map_err(|e| {
                QuicError::InvalidState(format!("Failed to acquire connection lock for cleanup: {e}"))
            })?;
    ...
    fn collect_stream_data(&self, conn: &mut quiche::Connection, stream_id: u64) -> Option<Vec<u8>> {
        let mut buf = [0; 4096];
    ...
    async fn process_stream_request_async(
        ...
        if let Ok(mut conn) = controller.conn.lock() {
            ...
            warn!("❌ Failed to acquire connection lock for response on stream {}", stream_id);
        }
    }
