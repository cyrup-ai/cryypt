...
async fn handle_download_completion(mut ctx: DownloadCompletionContext<'_>) -> Result<FileTransferResult> {
...
async fn initialize_download_writer(
    ...
    Ok((writer, event_rx, total_file_size, last_progress_update, received_checksum, transfer_success))
}
...
pub(crate) async fn execute_download_streaming(
    ...
    // Initialize download file writer and tracking variables
    let (mut writer, mut event_rx, mut total_file_size, mut last_progress_update, mut received_checksum, mut transfer_success) = 
...
async fn setup_download_connection(addr: SocketAddr) -> Result<crate::quic_conn::QuicConnectionHandle> {
                        ...
                        if ctx.last_progress_update.elapsed() >= Duration::from_millis(100) {
                            if let Some(callback) = ctx.progress_callback {
                                let progress = FileProgress {
                                    bytes_transferred: *ctx.bytes_transferred,
                                    total_bytes: ctx.total_file_size.unwrap_or(*ctx.bytes_transferred),
                                    percent: ctx.total_file_size.map(|total| {
                                        #[allow(clippy::cast_precision_loss)]
                                        {
                                            (*ctx.bytes_transferred as f64 / total as f64 * 100.0).min(100.0)
                                        }
                                    }).unwrap_or(0.0),
