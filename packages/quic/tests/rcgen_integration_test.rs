...
async fn test_rcgen_basic_certificate_generation() {
    ...
    assert!(cert_der.len() > 200, "Certificate should be substantial size");
    
    // Verify key properties
    let key_pem = key_pair.serialize_pem();
    assert!(key_pem.contains("-----BEGIN PRIVATE KEY-----"), "Key should be in PEM format");
    assert!(key_pem.contains("-----END PRIVATE KEY-----"), "Key should be in PEM format");
...
async fn test_rcgen_integration_with_tls_builder() {
    ...
    assert!(response.success, "Certificate generation should succeed: {:?}", response.issues);
    assert!(response.certificate_pem.is_some(), "Should have certificate PEM");
    assert!(response.private_key_pem.is_some(), "Should have private key PEM");
    
    let cert_pem = response.certificate_pem.unwrap();
    let key_pem = response.private_key_pem.unwrap();
    
    assert!(cert_pem.contains("-----BEGIN CERTIFICATE-----"), "Should be valid PEM certificate");
    assert!(key_pem.contains("-----BEGIN PRIVATE KEY-----"), "Should be valid PEM private key");
...
async fn test_quic_certificate_provider_integration() {
    ...
    let provider = QuicheCertificateProvider::create_self_signed("test-quic", temp_dir.clone()).await;
    ...
    match provider {
        Ok(provider) => {
            ...
            assert!(cert_pem.contains("-----BEGIN CERTIFICATE-----"), "Should have valid certificate PEM");
