...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("Single compression - compressed data length: {}", compressed.len());
    ...
    let mut stream = Cryypt::compress()
        ...
        .on_chunk(|result| match result {
            Ok(compressed_chunk) => {
                ...
                println!("Writing compressed chunk to archive: {} bytes", compressed_chunk.len());
                compressed_chunk
            }
            Err(e) => {
    ...
    println!("Streaming compression completed - total size: {} bytes", total_compressed_size);
