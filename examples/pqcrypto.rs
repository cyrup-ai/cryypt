...
async fn kem_examples() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for (name, builder) in [
        ...
        println!("    - Shared secret: {} bytes", encapsulation.shared_secret().as_bytes().len());
        println!("    - Ciphertext: {} bytes", encapsulation.ciphertext_size());
        
        // Bob decapsulates
        let decapsulation = match name {
            "ML-KEM-512" => KemBuilder::ml_kem_512(),
            "ML-KEM-768" => KemBuilder::ml_kem_768(),
            "ML-KEM-1024" => KemBuilder::ml_kem_1024(),
            _ => unreachable!(),
...
async fn signature_examples() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for (name, builder) in [
        ...
        assert!(verification.is_valid());
        println!("  ✓ Signature verified (compact: {} bytes)", signature.signature_size());
    }
    ...
    for variant in &sphincs_variants[..2] { // Test first 2 for brevity
...
async fn hybrid_encryption_example() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let plaintext = "This is a confidential document that needs post-quantum protection. ".repeat(50);
    ...
    println!("  - KEM ciphertext: {} bytes", encapsulation.ciphertext_size());
    println!("  - AES ciphertext: {} bytes", aes_ciphertext.to_bytes().len());
...
async fn key_management_example() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert_eq!(encap.shared_secret().as_bytes(), decap.shared_secret().as_bytes());
...
async fn performance_comparison() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for (name, builder) in [
        ...
        println!("  {} - KeyGen: {:?}, Sign: {:?}, Verify: {:?}, Size: {} bytes", 
                 name, keygen_time, sign_time, verify_time, signature.signature_size());
    }
...
async fn error_handling_examples() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match KemBuilder::ml_kem_512().with_public_key(invalid_key) {
        Err(CryptError::InvalidKeySize { expected, actual }) => {
            println!("  ✓ Correctly caught invalid KEM key size: expected {}, got {}", expected, actual);
        }
    ...
    match SignatureBuilder::ml_dsa_44().with_public_key(vec![0u8; 50]) {
        Err(CryptError::InvalidKeySize { expected, actual }) => {
            println!("  ✓ Correctly caught invalid signature key size: expected {}, got {}", expected, actual);
        }
