...
async fn test_aes_ciphertext_from_base64() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let ciphertext = Cipher::aes()
        .with_key(
            Key::size(256.bits())
                .with_store(FileKeyStore::at("/tmp/aes_test_cipher_base64").with_master_key(master_key))
                .with_namespace("test")
                .version(1),
        )
        .with_data(test_data)
...
async fn test_aes_ciphertext_from_hex() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let ciphertext = Cipher::aes()
        .with_key(
            Key::size(256.bits())
                .with_store(FileKeyStore::at("/tmp/aes_test_cipher_hex").with_master_key(master_key))
                .with_namespace("test")
                .version(1),
        )
        .with_data(test_data)
...
async fn test_aes_different_keys() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let ciphertext = Cipher::aes()
        .with_key(
            Key::size(256.bits())
                .with_store(FileKeyStore::at("/tmp/aes_test_diff_keys").with_master_key(master_key1))
                .with_namespace("test")
                .version(1),
        )
        .with_data(test_data)
    ...
    let result = Cipher::aes()
        .with_key(
            Key::size(256.bits())
                .with_store(FileKeyStore::at("/tmp/aes_test_diff_keys").with_master_key(master_key2))
                .with_namespace("test")
                .version(1),
        )
        .with_ciphertext(ciphertext.to_bytes())
