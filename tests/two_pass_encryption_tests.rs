...
async fn test_aes_then_chacha_two_pass() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let ciphertext = Cipher::aes()
        .with_key(
            Key::size(256.bits())
                .with_store(FileKeyStore::at("/tmp/two_pass_test_aes_chacha").with_master_key(master_key))
                .with_namespace("test")
                .version(1),
        )
        .with_data(test_data)
...
async fn test_chacha_then_aes_two_pass() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let ciphertext = Cipher::chachapoly()
        .with_key(
            Key::size(256.bits())
                .with_store(FileKeyStore::at("/tmp/two_pass_test_chacha_aes").with_master_key(master_key))
                .with_namespace("test")
                .version(1),
        )
        .with_data(test_data)
...
async fn test_aes_double_pass() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let ciphertext = Cipher::aes()
        .with_key(
            Key::size(256.bits())
                .with_store(FileKeyStore::at("/tmp/two_pass_test_aes_double").with_master_key(master_key))
                .with_namespace("test")
                .version(1),
        )
        .with_data(test_data)
...
async fn test_chacha_double_pass() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let ciphertext = Cipher::chachapoly()
        .with_key(
            Key::size(256.bits())
                .with_store(FileKeyStore::at("/tmp/two_pass_test_chacha_double").with_master_key(master_key))
                .with_namespace("test")
                .version(1),
        )
        .with_data(test_data)
...
async fn test_two_pass_with_encoding() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let base64_ciphertext = Cipher::aes()
        .with_key(
            Key::size(256.bits())
                .with_store(FileKeyStore::at("/tmp/two_pass_test_encoding").with_master_key(master_key))
                .with_namespace("test")
                .version(1),
        )
        .with_data(test_data)
...
async fn test_two_pass_large_data() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let ciphertext = Cipher::aes()
        .with_key(
            Key::size(256.bits())
                .with_store(FileKeyStore::at("/tmp/two_pass_test_large").with_master_key(master_key))
                .with_namespace("test")
                .version(1),
        )
...
async fn test_two_pass_empty_data() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let ciphertext = Cipher::aes()
        .with_key(
            Key::size(256.bits())
                .with_store(FileKeyStore::at("/tmp/two_pass_test_empty").with_master_key(master_key))
                .with_namespace("test")
                .version(1),
        )
...
async fn test_two_pass_wrong_order_fails() {
    ...
    let ciphertext = Cipher::aes()
        .with_key(
            Key::size(256.bits())
                .with_store(FileKeyStore::at("/tmp/two_pass_test_wrong_order").with_master_key(master_key))
                .with_namespace("test")
                .version(1),
        )
        .with_data(test_data)
...
async fn test_two_pass_different_namespaces() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let ciphertext = Cipher::aes()
        .with_key(
            Key::size(256.bits())
                .with_store(FileKeyStore::at("/tmp/two_pass_test_namespaces").with_master_key(master_key))
