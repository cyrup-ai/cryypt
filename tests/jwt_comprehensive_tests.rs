...
async fn test_expiry_validation() {
    ...
    let gen_with_leeway =
        Generator::new(Hs256Key::random()).with_validation_options(validation_with_leeway);
...
async fn test_not_before_validation() {
    ...
    let generator = Generator::new(Hs256Key::random()).with_validation_options(validation_opts);
...
async fn test_parallel_token_operations() {
    ...
    for token in tokens {
        let gen_clone = generator.clone();
        let handle = tokio::spawn(async move { gen_clone.verify(&token).await });
