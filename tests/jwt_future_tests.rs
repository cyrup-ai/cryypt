...
use chrono::Duration;
use cryypt::jwt::*;
use futures::future::{FutureExt, select, Either};
use futures::pin_mut;
use serde_json::json;
...
async fn test_future_join_combinations() {
    ...
    let (hs_token, es_token) = tokio::join!(
        hs_gen.token(&claims),
        es_gen.token(&claims)
    );
    ...
    let (hs_verify, es_verify) = tokio::join!(
        hs_gen.verify(&hs_token),
        es_gen.verify(&es_token)
    );
    ...
    let (hs_cross, es_cross) = tokio::join!(
        hs_gen.verify(&es_token),
        es_gen.verify(&hs_token)
    );
