...
use cryypt::pqcrypto::{
    SignatureAlgorithm,
    api::{SignatureKeyPairBuilder, SignBuilder, VerifyBuilder, MessageBuilder, SignatureDataBuilder},
};
use cryypt::prelude::*;
use std::fs;
...
async fn test_signature_file_operations() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let pk_data = tokio::fs::read(&pk_path).await?;
    let sk_data = tokio::fs::read(&sk_path).await?;

    // Sign message from file
    let signature = SignatureBuilder::ml_dsa_44()
        .with_secret_key(sk_data)?
...
async fn test_signature_wrong_public_key() -> Result<(), Box<dyn std::error::Error>> {
    let keypair1 = SignatureBuilder::falcon_512().generate().await?;
    let sk1 = keypair1.secret_key_vec();

    let keypair2 = SignatureBuilder::falcon_512().generate().await?;
    ...
    let verification = SignatureBuilder::falcon_512()
        .with_public_key(keypair2.public_key_vec())?
        .with_message(message)
        .with_signature(signature.signature())
        .verify()
        .await?;
...
async fn test_signature_error_invalid_key_size() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let result = SignatureBuilder::ml_dsa_65().with_public_key(invalid_key);
...
async fn test_signature_algorithm_metadata() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert_eq!(alg.secret_key_size(), 2560);
    ...
    assert_eq!(alg.secret_key_size(), 4032);
...
async fn test_dynamic_security_level_selection() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let keypair1 = SignatureBuilder::ml_dsa(44)?.generate().await?;
    ...
    let keypair3 = SignatureBuilder::falcon(512)?.generate().await?;
    ...
    // Test FALCON keypairs
    for keypair in [keypair3, keypair4] {
        let pk = keypair.public_key_vec();
        let sk = keypair.secret_key_vec();
...
async fn test_cross_algorithm_failure() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let keypair_mldsa = SignatureBuilder::ml_dsa_44().generate().await?;

    // Try to use FALCON-512 with ML-DSA-44 public key
    let result = SignatureBuilder::falcon_512()
        .with_public_key(keypair_mldsa.public_key_vec());
