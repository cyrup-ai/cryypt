...
async fn test_ml_kem_512_basic_encapsulation() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Extract keys before using the builder
    let pk = keypair.public_key_vec();
    let sk = keypair.secret_key_vec();

    // Encapsulate with public key
    let encapsulation_result = KemBuilder::ml_kem_512()
        .with_public_key(pk)?
        .encapsulate()
        .await?;

    // Decapsulate with secret key
    let decapsulation_result = KemBuilder::ml_kem_512()
        .with_secret_key(sk)?
...
async fn test_ml_kem_768_basic_encapsulation() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Encapsulate with public key
    let encapsulation_result = KemBuilder::ml_kem_768()
        .with_public_key(pk)?
        .encapsulate()
        .await?;

    // Decapsulate with secret key
    let decapsulation_result = KemBuilder::ml_kem_768()
        .with_secret_key(sk)?
        .with_ciphertext(encapsulation_result.ciphertext())
        .decapsulate()
        .await?;
...
async fn test_ml_kem_1024_basic_encapsulation() -> Result<(), Box<dyn std::error::Error>> {
    // Generate key pair
    let keypair = KemBuilder::ml_kem_1024().generate().await?;
    ...
    // Encapsulate with public key
    let encapsulation_result = KemBuilder::ml_kem_1024()
        .with_public_key(pk)?
        .encapsulate()
        .await?;

    // Decapsulate with secret key
    let decapsulation_result = KemBuilder::ml_kem_1024()
        .with_secret_key(sk)?
        .with_ciphertext(encapsulation_result.ciphertext())
        .decapsulate()
        .await?;
...
async fn test_ml_kem_hex_encoding() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Load keys from hex  
    let loaded_pk = KemBuilder::ml_kem_512()
        .with_public_key_hex(&pk_hex)?;
    let loaded_sk = KemBuilder::ml_kem_512()
        .with_secret_key_hex(&sk_hex)?;

    // Encapsulate
    let encapsulation_result = loaded_pk.encapsulate().await?;

    // Get ciphertext as hex
    let ct_hex = encapsulation_result.ciphertext_hex();

    // Decapsulate using hex ciphertext
    let decapsulation_result = loaded_sk
...
async fn test_ml_kem_base64_encoding() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let pk_base64 = base64::engine::general_purpose::STANDARD
        .encode(keypair.public_key());
    let sk_base64 = base64::engine::general_purpose::STANDARD
        .encode(keypair.secret_key());

    // Load keys from base64
    let loaded_pk = KemBuilder::ml_kem_1024()
        .with_public_key_base64(&pk_base64)?;
    let loaded_sk = KemBuilder::ml_kem_1024()
...
async fn test_ml_kem_file_operations() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Extract keys for reuse
    let loaded_pk = loaded_keypair.public_key_vec();
    let loaded_sk = loaded_keypair.secret_key_vec();

    // Encapsulate
    let encapsulation_result = KemBuilder::ml_kem_768()
        .with_public_key(loaded_pk)?
        .encapsulate()
        .await?;
...
async fn test_ml_kem_dynamic_security_level() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for level in security_levels {
        let keypair = KemBuilder::ml_kem(level)?.generate().await?;
        
        // Extract keys before using
        let pk = keypair.public_key_vec();
        let sk = keypair.secret_key_vec();

        let encapsulation_result = KemBuilder::ml_kem(level)?
            .with_public_key(pk)?
            .encapsulate()
            .await?;

        let decapsulation_result = KemBuilder::ml_kem(level)?
            .with_secret_key(sk)?
            .with_ciphertext(encapsulation_result.ciphertext())
            .decapsulate()
            .await?;
...
async fn test_ml_kem_shared_secret_serialization() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let encapsulation_result = keypair.encapsulate().await?;
    ...
    assert_eq!(encapsulation_result.shared_secret(), &ss_from_json);
...
async fn test_ml_kem_error_invalid_key_size() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let result = KemBuilder::ml_kem_768().with_public_key(invalid_key);
...
async fn test_ml_kem_cross_algorithm_failure() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let keypair_512 = KemBuilder::ml_kem_512().generate().await?;

    // Try to use ML-KEM-768 with ML-KEM-512 public key
    let result =
        KemBuilder::ml_kem_768().with_public_key(keypair_512.public_key_vec());
