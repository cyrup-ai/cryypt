...
use cryypt::prelude::*;
use cryypt::pqcrypto::{KemAlgorithm, SignatureAlgorithm};
...
async fn example_basic_kem_key_exchange() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let bob_keypair = KemBuilder::ml_kem_768()
        .generate()
        .await?;
    
    // Bob shares his public key with Alice
    let bob_public_key = bob_keypair.public_key.clone().ok_or("No public key")?;
    println!("Bob shares his public key (size: {} bytes)", bob_public_key.len());
    ...
    println!("Alice's shared secret: {}", alice_encapsulation.shared_secret().to_hex());
    println!("Ciphertext size: {} bytes", alice_encapsulation.ciphertext_size());
    ...
    let bob_decapsulation = bob_keypair
        .with_ciphertext(ciphertext)
        .decapsulate()
        .await?;
    
    println!("Bob's shared secret: {}", bob_decapsulation.shared_secret().to_hex());
...
async fn example_hybrid_encryption() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let kem_keypair = KemBuilder::ml_kem_512()
        .generate()
        .await?;
    ...
    println!("KEM ciphertext size: {} bytes", encapsulation.ciphertext_size());
    println!("AES ciphertext size: {} bytes", aes_ciphertext.to_bytes().len());
    
    // --- Decryption (Receiver) ---
    println!("\n--- Receiver decrypts ---");
    
    // 1. Decapsulate to recover shared secret
    let decapsulation = kem_keypair
        .with_ciphertext(encapsulation.ciphertext())
        .decapsulate()
        .await?;
    
    // 2. Use shared secret as AES key to decrypt
    let decrypted_message = Cipher::aes()
        .with_key(
            Key::from_bytes(decapsulation.shared_secret().as_bytes())
                .with_namespace("hybrid-pq")
        )
    ...
    println!("Decrypted message: {:?}", std::str::from_utf8(&decrypted_message)?);
...
async fn example_document_signing() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let signing_keypair = SignatureBuilder::ml_dsa_65()
        .generate()
        .await?;
    ...
    println!("\nVerifying signature...");
    let verification = signing_keypair
        .with_message_text(document)
    ...
    let tampered_verification = signing_keypair
...
async fn example_algorithm_comparison() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for algorithm in [KemAlgorithm::MlKem512, KemAlgorithm::MlKem768, KemAlgorithm::MlKem1024] {
        println!("\n{} (Security Level {})", algorithm, algorithm.security_level());
        ...
        // Time encapsulation
        let start = Instant::now();
        let encap = keypair.encapsulate().await?;
        let encap_time = start.elapsed();
        
        // Time decapsulation
        let start = Instant::now();
        let _decap = keypair.with_ciphertext(encap.ciphertext()).decapsulate().await?;
    ...
    for (name, builder) in [
        ...
        let start = Instant::now();
        let sig = keypair.with_message(test_message).sign().await?;
    ...
    let start = Instant::now();
    let sig = falcon_kp.with_message(test_message).sign().await?;
    let sign_time = start.elapsed();
    
    let start = Instant::now();
    let _verify = falcon_kp
        .with_message(test_message)
        .with_signature(sig.signature())
        .verify()
        .await?;
...
async fn example_key_serialization() -> Result<(), Box<dyn std::error::Error>> {
    ...
    tokio::fs::write(&kem_pk_path, &kem_kp.public_key.clone().ok_or("No public key")?).await?;
    tokio::fs::write(&kem_sk_path, &kem_kp.secret_key.clone().ok_or("No secret key")?).await?;
    ...
    let sig_pk_b64 = base64::engine::general_purpose::STANDARD
        .encode(&sig_kp.public_key.clone().ok_or("No public key")?);
    let sig_sk_b64 = base64::engine::general_purpose::STANDARD
        .encode(&sig_kp.secret_key.clone().ok_or("No secret key")?);
    ...
    // Load KEM keys
    let loaded_kem = KemBuilder::ml_kem_768()
        .with_keypair_files(&kem_pk_path, &kem_sk_path)
        .await?;
    
    let encap = loaded_kem.encapsulate().await?;
    ...
    let loaded_sig = SignatureBuilder::ml_dsa_65()
        .with_public_key_base64(&sig_pk_b64)?
...
async fn example_error_handling() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match KemBuilder::ml_kem_768().with_public_key(invalid_key) {
        ...
        Err(CryptError::InvalidKeySize { expected, actual }) => {
            println!("✓ Caught error: Expected {} bytes, got {}", expected, actual);
        }
    ...
    let sig = keypair.with_message(b"original").sign().await?;
    
    let verify_result = keypair
