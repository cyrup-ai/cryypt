...
pub trait KemKeyPairBuilder {
    ...
    fn with_public_key<T: Into<Vec<u8>>>(self, public_key: T) -> Result<Self::Output>
    where
        Self: Sized;
    
    /// Load secret key from bytes
    fn with_secret_key<T: Into<Vec<u8>>>(self, secret_key: T) -> Result<Self::Output>
    where
        Self: Sized;
    
    /// Load public key from hex
    fn with_public_key_hex(self, hex: &str) -> Result<Self::Output>
    where
        Self: Sized,
    {
    ...
    fn with_secret_key_hex(self, hex: &str) -> Result<Self::Output>
    where
        Self: Sized,
    {
        let bytes = hex::decode(hex)
    ...
    fn with_public_key_base64(self, base64: &str) -> Result<Self::Output>
    where
        Self: Sized,
    {
        use base64::Engine;
    ...
    fn with_secret_key_base64(self, base64: &str) -> Result<Self::Output>
    where
        Self: Sized,
    {
        use base64::Engine;
...
pub trait CiphertextBuilder {
    ...
    {
        let bytes = hex::decode(hex)
            .map_err(|e| CryptError::InvalidEncryptedData(format!("Invalid hex ciphertext: {}", e)))?;
        ...
        Self: Sized,
    {
        ...
        let bytes = base64::engine::general_purpose::STANDARD
            .decode(base64)
            .map_err(|e| CryptError::InvalidEncryptedData(format!("Invalid base64 ciphertext: {}", e)))?;
        Ok(self.with_ciphertext(bytes))
    }
...
pub trait SignatureDataBuilder {
        ...
        Self: Sized,
    {
        ...
        let bytes = base64::engine::general_purpose::STANDARD
            .decode(base64)
            .map_err(|e| CryptError::InvalidParameters(format!("Invalid base64 signature: {}", e)))?;
        Ok(self.with_signature(bytes))
    }
