...
mod tests {
    ...
    async fn test_revocation_basic() {
        ...
        // Revoke the token
        revocation.revoke(&token, "Test revocation");
        assert!(revocation.is_revoked(&token));
        ...
        let info = revocation.get_revocation_info(&token).unwrap();
        assert_eq!(info.reason, "Test revocation");
    }
    ...
    async fn test_revocation_cleanup() {
        ...
        assert_eq!(revocation.revoked_count(), 1);
        
        // Wait for token to expire and cleanup to run
        sleep(std::time::Duration::from_millis(300)).await;
        
        // Token should be cleaned up
        assert_eq!(revocation.revoked_count(), 0);
    ...
    #[tokio::test]
    async fn test_revocation_invalid_token() {
        let key = Hs256Key::random();
        let revocation = Revocation::wrap(key);
        
        // Revoking invalid tokens should not panic
        revocation.revoke("invalid", "Test");
        revocation.revoke("invalid.token", "Test");
        revocation.revoke("", "Test");
        
        // Should still be tracked
        assert!(revocation.is_revoked("invalid"));
        assert!(revocation.is_revoked("invalid.token"));
        assert!(revocation.is_revoked(""));
    }

    #[tokio::test]
    async fn test_revocation_multiple_tokens() {
        let key = Hs256Key::random();
        let revocation = Revocation::wrap(key);
        ...
        // Revoke some tokens
        revocation.revoke(&tokens[0], "Reason 0");
        revocation.revoke(&tokens[2], "Reason 2");
        revocation.revoke(&tokens[4], "Reason 4");
        
        // Check which are revoked
        assert!(revocation.is_revoked(&tokens[0]));
        assert!(!revocation.is_revoked(&tokens[1]));
        assert!(revocation.is_revoked(&tokens[2]));
        assert!(!revocation.is_revoked(&tokens[3]));
        assert!(revocation.is_revoked(&tokens[4]));
        
        // Verify non-revoked tokens still work
        let result1 = revocation.verify(&tokens[1]).await;
        assert!(result1.is_ok());
        
        let result3 = revocation.verify(&tokens[3]).await;
