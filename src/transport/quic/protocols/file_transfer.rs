...
impl FileTransferServerBuilder {
    ...
    pub fn listen(self, addr: &str) -> impl AsyncQuicResult<FileTransferServer> {
        ...
        async move {
            ...
            let (cert_path, key_path) = if let (Some(cert), Some(key)) = (self.cert_path, self.key_path) {
                (cert, key)
            } else {
...
impl FileTransferServer {
    ...
    pub fn get_transfer_stats() -> impl AsyncQuicResult<Vec<FileTransferProgress>> {
        async move {
            // Implementation would return active transfer stats
            Ok(vec![])
        }
    }
}
...
impl FileUploadBuilder {
    ...
    pub fn execute_with_stream(self) -> (impl AsyncQuicResult<TransferResult>, impl Stream<Item = FileTransferProgress>) {
        ...
        (upload_future, tokio_stream::wrappers::UnboundedReceiverStream::new(progress_rx))
    }
}
