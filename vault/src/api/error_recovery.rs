...
impl ErrorRecovery {
    ...
    pub fn production_optimized() -> Self {
        Self {
            ...
            circuit_recovery_timeout: Duration::from_secs(15),
        }
    }
    ...
    fn is_database_error_retryable(&self, db_err: &surrealdb::Error) -> bool {
        ...
        match db_err {
            Error::Db(DbError::TxFailure) => true,        // Transaction conflicts - retryable
            Error::Db(DbError::QueryTimedout) => true,    // Timeouts - retryable
            Error::Db(DbError::TxReadonly) => false,      // Readonly mode - permanent
            _ => false, // Conservative default for unknown database errors
        }
    }
    ...
    pub fn get_retry_policy(&self, error: &VaultError) -> &RetryPolicy {
        match error {
            ...
            VaultError::Io(_) | VaultError::MemoryProtection(_) | VaultError::Provider(_) => &self.system_policy,
    ...
    pub async fn retry_with_policy<F, T, Fut>(&self, operation: F, policy: &RetryPolicy) -> Result<T, VaultError>
    where
        F: Fn() -> Fut,
            ...
            match operation().await {
                ...
                Err(error) => {
                    ...
                    if attempts >= policy.max_attempts {
                        error!("Max retry attempts ({}) exceeded for error: {}", policy.max_attempts, error);
                        return Err(error);
                    }
                    ...
                    debug!("Retrying operation (attempt {}/{}) after {:?} delay: {}", 
                           attempts + 1, policy.max_attempts, actual_delay, error);
