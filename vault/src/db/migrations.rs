...
async fn create_essential_schema(db: &Surreal<Any>) -> Result<(), VaultError> {
    ...
    const VAULT_ENTRIES_SCHEMA: &str = r#"
        ...
        DEFINE FIELD ttl ON TABLE vault_entries TYPE option<datetime>;
...
/// Run structured migrations using surrealdb-migrations
async fn run_structured_migrations(db: &Surreal<Any>) -> Result<(), VaultError> {
    debug!("Running structured migrations");
    ...
    match runner.up().await {
        Ok(_) => {
            info!("Structured migrations completed");
            Ok(())
        }
        Err(e) => {
            warn!(error = %e, "Structured migration warning - essential schema still available");
            // Don't fail completely since we have essential schema
            Ok(())
        }
    }
}
