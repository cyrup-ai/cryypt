...
impl JwtBuilder {
    ...
    fn on_chunk_impl<F>(self, handler: F) -> JwtBuilderWithChunkHandler<F>
    where
        F: Fn(JwtResult<Vec<u8>>) -> Vec<u8> + Send + 'static,
    {
        JwtBuilderWithChunkHandler {
            algorithm: self.algorithm,
            secret: self.secret,
            private_key: self.private_key,
            public_key: self.public_key,
    ...
    pub fn on_chunk<F>(self, handler: F) -> JwtBuilderWithChunkHandler<F>
    where
        F: Fn(JwtResult<Vec<u8>>) -> Vec<u8> + Send + 'static,
    {
    ...
    F: Fn(JwtResult<Vec<u8>>) -> Vec<u8> + Send + 'static,
{
    ...
    pub async fn sign<C: Serialize + Send + 'static>(self, claims: C) -> Vec<u8> {
        ...
        let converted_result = result.map(|s| s.into_bytes()).map_err(|e| e.into());
    ...
    pub async fn verify<S: AsRef<str>>(self, token: S) -> Vec<u8> {
        ...
        let converted_result = result.map(|v| v.to_string().into_bytes()).map_err(|e| e.into());
        (self.result_handler)(converted_result)
    }
}
    ...
    F: Fn(JwtResult<Vec<u8>>) -> Vec<u8> + Send + 'static,
{
    ...
    pub fn sign_stream<C: Serialize + Send + Clone + 'static>(
        ...
        futures::stream::unfold((algorithm, claims, secret, private_key, handler, false), 
            move |(algorithm, claims, secret, private_key, handler, done)| async move {
                ...
                Some((processed_chunk, (algorithm_clone, claims_clone, secret_clone, private_key_clone, handler, true)))
            })
    }
    ...
    pub fn verify_stream<S: AsRef<str> + Send + Clone + 'static>(
        ...
        futures::stream::unfold((token, secret, public_key, handler, false), 
            move |(token, secret, public_key, handler, done)| async move {
                ...
                Some((processed_chunk, (token_clone, secret_clone, public_key_clone, handler, true)))
            })
    }
}
    ...
    fn poll(
        ...
        match receiver.poll(cx) {
            ...
            std::task::Poll::Ready(Err(_)) => {
                std::task::Poll::Ready(Err(JwtError::internal("JWT operation failed")))
            }
    ...
    fn poll(
        ...
        match receiver.poll(cx) {
            ...
            std::task::Poll::Ready(Err(_)) => std::task::Poll::Ready(Err((self.error_handler)(
                JwtError::internal("JWT operation failed"),
            ))),
            std::task::Poll::Pending => std::task::Poll::Pending,
        }
    }
...
async fn sign_jwt<C: Serialize + Send + 'static>(
    ...
    std::thread::spawn(move || {
        let result = (|| {
            ...
            let signature = match algorithm.as_str() {
                "HS256" => {
                    let secret =
                        secret.ok_or_else(|| JwtError::missing_key("Secret required for HS256"))?;
                    sign_hs256(&message, &secret)?
                }
                "HS384" => {
                    let secret =
                        secret.ok_or_else(|| JwtError::missing_key("Secret required for HS384"))?;
                    sign_hs384(&message, &secret)?
                }
                "HS512" => {
                    let secret =
                        secret.ok_or_else(|| JwtError::missing_key("Secret required for HS512"))?;
                    sign_hs512(&message, &secret)?
                }
                "RS256" => {
                    let key = private_key
                        .ok_or_else(|| JwtError::missing_key("Private key required for RS256"))?;
                    sign_rs256(&message, &key)?
                }
                "RS384" => {
                    let key = private_key
                        .ok_or_else(|| JwtError::missing_key("Private key required for RS384"))?;
                    sign_rs384(&message, &key)?
                }
                "RS512" => {
                    let key = private_key
                        .ok_or_else(|| JwtError::missing_key("Private key required for RS512"))?;
                    sign_rs512(&message, &key)?
                }
                "ES256" => {
                    let key = private_key
                        .ok_or_else(|| JwtError::missing_key("Private key required for ES256"))?;
                    sign_es256(&message, &key)?
                }
                "ES384" => {
                    let key = private_key
                        .ok_or_else(|| JwtError::missing_key("Private key required for ES384"))?;
    ...
    rx.await
        .map_err(|_| JwtError::internal("JWT signing task failed"))?
}
...
async fn verify_jwt(
            ...
            let valid = match header.alg.as_str() {
                "HS256" => {
                    let secret = secret.ok_or_else(|| {
                        JwtError::missing_key("Secret required for HS256 verification")
                    })?;
                    verify_hs256(&message, &signature, &secret)?
                }
                "HS384" => {
                    let secret = secret.ok_or_else(|| {
                        JwtError::missing_key("Secret required for HS384 verification")
                    })?;
                    verify_hs384(&message, &signature, &secret)?
                }
                "HS512" => {
                    let secret = secret.ok_or_else(|| {
                        JwtError::missing_key("Secret required for HS512 verification")
                    })?;
                    verify_hs512(&message, &signature, &secret)?
                }
                "RS256" => {
                    let key = public_key.ok_or_else(|| {
                        JwtError::missing_key("Public key required for RS256 verification")
                    })?;
                    verify_rs256(&message, &signature, &key)?
                }
                "RS384" => {
                    let key = public_key.ok_or_else(|| {
                        JwtError::missing_key("Public key required for RS384 verification")
                    })?;
                    verify_rs384(&message, &signature, &key)?
                }
                "RS512" => {
                    let key = public_key.ok_or_else(|| {
                        JwtError::missing_key("Public key required for RS512 verification")
                    })?;
                    verify_rs512(&message, &signature, &key)?
                }
                "ES256" => {
                    let key = public_key.ok_or_else(|| {
                        JwtError::missing_key("Public key required for ES256 verification")
                    })?;
                    verify_es256(&message, &signature, &key)?
                }
                "ES384" => {
                    let key = public_key.ok_or_else(|| {
                        JwtError::missing_key("Public key required for ES384 verification")
                    })?;
    ...
    rx.await
        .map_err(|_| JwtError::internal("JWT verification task failed"))?
}
...
fn verify_es256(message: &str, signature: &[u8], public_key_der: &[u8]) -> Result<bool, JwtError> {
    ...
    let signature = Signature::from_der(signature).map_err(|_e| JwtError::invalid_signature())?;
...
// Removed public macro: on_result! (no exposed macros policy)
