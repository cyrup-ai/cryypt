...
impl ClosureCallTracker {
    ...
    fn mark_called(&self, error_msg: Option<String>) {
        ...
        *self.last_error.lock().unwrap() = error_msg;
    }
}
...
async fn test_cipher_aes_error_handler_usage() {
    ...
    assert_eq!(tracker.call_count(), 1, "Error handler should be called exactly once");
    assert!(tracker.last_error().is_some(), "Error should have been captured");
    assert_eq!(result, b"ERROR_HANDLER_CALLED".to_vec(), "Error handler return value should be used");
    
    println!("✅ AES Error Handler Test PASSED - Closure was called with error: {:?}", tracker.last_error());
}
...
async fn test_cipher_aes_success_handler_usage() {
    ...
    assert!(tracker.was_called(), "Success handler closure was not called");
    assert_eq!(tracker.call_count(), 1, "Success handler should be called exactly once");
    assert!(tracker.last_error().is_none(), "No error should have been captured in success case");
    assert!(result.ends_with(b"_HANDLER_CALLED"), "Success handler should have modified the result");
...
async fn test_cipher_chacha_error_handler_usage() {
    ...
    // Verify the closure was called and handled the error
    assert!(tracker.was_called(), "ChaCha error handler closure was not called");
    assert_eq!(result, b"CHACHA_ERROR_HANDLER_CALLED".to_vec(), "ChaCha error handler return value should be used");
    
    println!("✅ ChaCha Error Handler Test PASSED - Closure was called with error: {:?}", tracker.last_error());
}
...
async fn test_hash_sha256_error_handler_usage() {
    ...
    // Verify the closure was called and modified the result
    assert!(tracker.was_called(), "SHA256 handler closure was not called");
    assert!(result.ends_with(b"_SHA256_HANDLER_CALLED"), "SHA256 handler should have modified the result");
...
async fn test_hash_blake2b_error_handler_usage() {
    ...
    // Verify the closure was called and modified the result
    assert!(tracker.was_called(), "BLAKE2B handler closure was not called");
    assert!(result.ends_with(b"_BLAKE2B_HANDLER_CALLED"), "BLAKE2B handler should have modified the result");
...
async fn test_hash_sha3_error_handler_usage() {
    ...
    assert!(result.ends_with(b"_SHA3_HANDLER_CALLED"), "SHA3 handler should have modified the result");
